...
import { DocumentPrimitive, DocumentValue, QueryValue } from './types.ts';
import { isArray, isBoolean, isNull, isNumber, isObject, isString, isUndefined } from './utils.ts';
...
export function someElementMatch(...queries: QueryValue[]) {
	return (target: Readonly<DocumentValue>) =>
		isArray(target) &&
		target.some((targetValue) => queries.every((query) => matchValues(query, targetValue)));
}
...
export function everyElementMatch(...queries: QueryValue[]) {
	return (target: Readonly<DocumentValue>) =>
		isArray(target) &&
		target.every((targetValue) => queries.every((query) => matchValues(query, targetValue)));
}
