...
import { deserializeStorage, executeProjection, findMultipleDocuments, findOneDocument, updateDocument } from './core.ts';
import { Acceptable, DatabaseConfig, Document, Options, Projection, Query, QueryFunction, Update, UpdateFunction } from './types.ts';
import { cleanArray, deepClone, isArray, isBoolean, isFunction, isNull, isObject, isObjectEmpty, isString, isUndefined, sanitizeObject } from './utils.ts';
...
export class Database<Schema extends Acceptable<Schema> = Document> {
	...
	constructor(config?: Partial<DatabaseConfig> | string) {
		...
		if (isString(config)) {
			config = { path: config, autoload: true, autosave: true };
		}
		if (!isObject(config)) {
			throw new TypeError('Config must be an object or a string');
		}
	...
	public async insertOne(
		...
		if (options && isBoolean(options.immutable)) {
	...
	public async findOne(
		...
		options?: Partial<Options>,
		projection?: Projection<Schema>,
	): Promise<Schema | Partial<Schema> | null> {
		if (!isUndefined(query) && !isObject(query) && !isFunction(query)) {
			throw new TypeError('Query must be an object or function');
		}
		...
		const found: number | null = findOneDocument<Schema>(
			query,
			this.documents,
		);
		...
		if (isObject(projection)) {
			return executeProjection<Schema>(document, projection);
		}
		return document;
	}
	...
	public async findMany(
		...
		const found: number[] = findMultipleDocuments<Schema>(
			query,
			this.documents,
		);
	...
	public async updateOne(
		...
		if (!isObject(update) && !isFunction(update)) {
